...
TELEMAC_DIR = telemac_example("malpasset")
...
def main():
    ...
    m = read_selafin(MESH)
    assert m["npoin"] == 53081 and m["varnames"][0] == "BOTTOM", (
        m["npoin"],
        m["varnames"],
    )
    ...
    xg, yg = np.meshgrid(
        x0 + dx * np.arange(nx), y0 + dx * np.arange(ny), indexing="ij"
    )
    ...
    ik = m["ikle"]
    area = 0.5 * np.abs(
        (x[ik[:, 1]] - x[ik[:, 0]]) * (y[ik[:, 2]] - y[ik[:, 0]])
        - (x[ik[:, 2]] - x[ik[:, 0]]) * (y[ik[:, 1]] - y[ik[:, 0]])
    )
    vol_mesh = float((area * h_node[ik].mean(axis=1)).sum())
    print(
        f"grid {nx} x {ny} @ {dx} m, origin ({x0:.1f}, {y0:.1f}); in-mesh cells {inmesh.sum()}"
    )
    print(
        f"bed in mesh: min {bed[inmesh].min():.2f} max {bed[inmesh].max():.2f} m; land fill {land:.1f} m"
    )
    print(
        f"reservoir cells {reservoir.sum()}, components {ncomp} (sizes {sorted(sizes, reverse=True)[:5]})"
    )
    print(
        f"dam-face kick cells {kick.sum()} (bed {bed[kick].min():.1f}..{bed[kick].max():.1f} m, "
        f"added volume {KICK_DEPTH * kick.sum() * dx * dx:.0f} m3)"
    )
    print(
        f"max initial depth {float((eta0 - bed).max()):.2f} m; sea cells (bed<{SEA_LEVEL} m, set to eta={SEA_LEVEL}) {sea.sum()}, "
        f"sea volume {float((SEA_LEVEL - bed[sea]).sum() * dx * dx) / 1e6:.2f} Mm3, components {ndimage.label(sea)[1]}"
    )
    print(
        f"reservoir volume: grid {vol_grid / 1e6:.3f} Mm3 vs mesh (P1 lumped) {vol_mesh / 1e6:.3f} Mm3"
    )
    for n, i, j in zip(GAUGE_NAMES, gi, gj):
        print(
            f"  {n:6s} ij=({i},{j}) bed {bed[i, j]:.2f} m  mesh-interp "
            f"{float(mtri.LinearTriInterpolator(tri, zb)(GAUGE_X[GAUGE_NAMES.index(n)], GAUGE_Y[GAUGE_NAMES.index(n)])):.2f}"
        )

    np.savez_compressed(
        args.workdir / "grid.npz",
        ...
        x0=x0,
        y0=y0,
        gauge_names=np.array(GAUGE_NAMES),
        gauge_x=GAUGE_X,
        gauge_y=GAUGE_Y,
        gauge_i=gi,
        gauge_j=gj,
    ...
    np.savetxt(
        args.workdir / "bathy.xyz",
        np.column_stack(
            [
                (dx * np.arange(nx))[:, None].repeat(ny, 1).ravel(),
                (dx * np.arange(ny))[None, :].repeat(nx, 0).ravel(),
                -bed.ravel(),
            ]
        ),
        fmt="%.3f",
