...
import matplotlib
from paths import celeris_out, telemac_example, telemac_ref  # noqa: F401
...
WORKDIR = celeris_out("malpasset")
TELEMAC = telemac_ref()
ARRIVAL_DEPTH = 0.05  # m: arrival = first time depth exceeds this
GAUGES = ["P6", "P7", "P8", "P9", "P10", "P11", "P12", "P13", "P14"]
MEASURED = dict(
    zip(GAUGES, [40.3, 14.6, 24.0, 12.8, 11.8, 8.3, 10.1, 6.8, 5.4])
)  # doc/malpasset.tex table
...
TELEMAC_RFO = {"hllc": {"A": 97.2, "AB": 1127.8, "AC": 1369.3}}
COL = {"measured": "#444444", "telemac": "#2a78d6", "celeris": "#eb6834"}
...
def telemac_metrics(npz, names, xs, ys):
    ...
    depth = np.array(
        [
            mtri.LinearTriInterpolator(tri, fs - d["bottom"], trifinder=finder)(
                xs, ys
            ).filled(np.nan)
            for fs in d["free_surface"]
        ]
    )  # (nt, npts)
    ...
    return {
        "t": t,
        "depth": depth,
        "hmax": np.nanmax(depth, axis=0),
        "hmax_nearest": depth_nn.max(axis=0),
        "arrival": {n: arrival(t, depth[:, k]) for k, n in enumerate(names)},
        "bed": mtri.LinearTriInterpolator(tri, d["bottom"], trifinder=finder)(
            xs, ys
        ).filled(np.nan),
    }
...
def main():
    ...
    for n in GAUGES:
        ...
        row = {
            ...
            "x": int(xs[k]),
            "y": int(ys[k]),
            "bed_celeris_m": round(bed[gi[k], gj[k]], 2),
            "measured_m": MEASURED[n],
            "celeris_m": round(float(r["hmax"][gi[k], gj[k]]), 2),
            "celeris_solver_envelope_m": round(
                float(r["hmax_solver"][gi[k], gj[k]]), 2
            ),
        }
    ...
    arows = [
        {"quantity": "arrival_A_s", "observed": "", "celeris": round(arr["A"], 1)},
        {
            "quantity": "A_to_B_s",
            "observed": OBSERVED_AB,
            "celeris": round(arr["B"] - arr["A"], 1),
        },
        {
            "quantity": "A_to_C_s",
            "observed": OBSERVED_AC,
            "celeris": round(arr["C"] - arr["A"], 1),
        },
    ]
    for tag, m in tel.items():
        a = m["arrival"]
    ...
    arows.append(
        {"quantity": "arrival_threshold_m", "observed": "", "celeris": ARRIVAL_DEPTH}
    )
    ...
    fig, (ax1, ax2) = plt.subplots(
        1, 2, figsize=(12, 4.6), gridspec_kw={"width_ratios": [2.2, 1]}
    )
    xg = np.arange(len(GAUGES))
    wbar = 0.26
    ...
    if "hllc" in tel:
        series.append(
            (
                "TELEMAC-2D HLLC",
                [tel["hllc"]["hmax"][idx[n]] for n in GAUGES],
                COL["telemac"],
            )
        )
    series.append(
        (
            "Celeris SWE",
            [rows[i]["celeris_m"] for i in range(len(GAUGES))],
            COL["celeris"],
        )
    )
    ...
    ax1.set_xticks(xg, GAUGES)
    ax1.set_ylabel("Maximum Water Depth (m)")
    ax1.set_title("Maximum Depth At Gauges P6-P14")
    ax1.legend(frameon=False)
    ax1.spines[["top", "right"]].set_visible(False)
    ax1.grid(axis="y", color="#e5e5e5", linewidth=0.6)
    ax1.set_axisbelow(True)

    labels = ["A To B", "A To C"]
    xa = np.arange(2)
    ...
    if "hllc" in tel:
        aser.append(
            (
                "TELEMAC-2D HLLC",
                [TELEMAC_RFO["hllc"]["AB"], TELEMAC_RFO["hllc"]["AC"]],
                COL["telemac"],
            )
        )
    aser.append(
        ("Celeris SWE", [arr["B"] - arr["A"], arr["C"] - arr["A"]], COL["celeris"])
    )
    ...
    ax2.set_xticks(xa, labels)
    ax2.set_ylabel("Travel Time (s)")
    ax2.set_ylim(0, 1.35 * max(max(v) for _, v, _ in aser))
    ax2.set_title(f"Wave Travel Time (Depth > {ARRIVAL_DEPTH:g} m)")
    ax2.legend(frameon=False, fontsize=8, loc="upper left")
    ax2.spines[["top", "right"]].set_visible(False)
    ax2.grid(axis="y", color="#e5e5e5", linewidth=0.6)
    ax2.set_axisbelow(True)
    fig.suptitle(
        f"Malpasset Dam Break: Celeris (dx = {float(r['dx']):g} m, Courant {float(r['courant']):g}, "
        f"n = {float(r['manning_n']):.4f}, Seed Wetting {float(r['seed_wetting']) if 'seed_wetting' in r else 0:g} m) Vs Reference Data",
        fontsize=11,
    )
    fig.tight_layout()
    fig.savefig(out / f"malpasset_comparison{args.tag}.png", dpi=150)
    plt.close(fig)

    print(
        f"{'gauge':6s} {'bed':>6s} {'measured':>9s} {'celeris':>8s} "
        + " ".join(f"{'tel_' + k:>9s}" for k in tel)
    )
    for row, n in zip(rows, GAUGES):
        print(
            f"{n:6s} {row['bed_celeris_m']:6.2f} {row['measured_m']:9.1f} {row['celeris_m']:8.2f} "
            + " ".join(f"{row[f'telemac_{k}_m']:9.2f}" for k in tel)
        )
    for row in arows:
        print(row)
    print(
        f"volume: t0 {r['volume'][0] / 1e6:.3f} Mm3, end {r['volume'][-1] / 1e6:.3f} Mm3 "
