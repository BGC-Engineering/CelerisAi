...
MANNING_N = (
    1.0 / 30.0
)  # TELEMAC Strickler 30 -> n = 1/30 (FrictionCalc: isManning=1 uses friction as n)
...
def main():
    ...
    ap.add_argument(
        "--steps", type=int, default=0, help="override step count (smoke test)"
    )
    ap.add_argument(
        "--sample-dt", type=float, default=1.0, help="readback interval (s)"
    )
    ap.add_argument(
        "--seed-wetting",
        type=float,
        default=0.0,
        help="seed depth (m) for the wetting kernel, 0 = off",
    )
    ...
    every = max(1, round(args.sample_dt / dt))
    print(
        f"grid {nx}x{ny} dx={dx} m | dt={dt:.4f} s | steps={nsteps} | base_depth={hmax0:.2f} m | "
        f"delta={delta:.4f} m | Manning n={MANNING_N:.4f} | seed_wetting={seed_h} m | readback every {every} steps",
        flush=True,
    )

    # volume = sum(eta - eta_ref): water on land + sea-surface anomaly; conserved by an exact scheme (walls all round)
    ts, gauge_eta, volume, hmax = (
        [0.0],
        [eta0[gi, gj]],
        [float((eta0 - eta_ref).sum() * dx * dx)],
        np.zeros_like(bed),
    )
    ...
    for n in range(1, nsteps + 1):
        ...
        if not np.isfinite(eta).all() or np.abs(eta).max() > BLOWUP_ETA:
            print(
                f"BLOW-UP at t={t:.1f} s step {n}: finite={np.isfinite(eta).all()} max|eta|={np.nanmax(np.abs(eta)):.3g}"
            )
            sys.exit(1)
        depth = eta - bed
        np.maximum(hmax, depth, out=hmax)
        ts.append(t)
        gauge_eta.append(eta[gi, gj])
        volume.append(float((eta - eta_ref).sum() * dx * dx))
        while frames_left and t >= frames_left[0] - 0.5 * every * dt:
            np.save(
                wd / f"frame{args.tag}_t{int(frames_left.pop(0)):04d}.npy",
                eta.astype(np.float32),
            )
        if n % (every * 200) == 0 or n == nsteps:
            el = time.time() - t0
            print(
                f"t={t:7.1f} s  wall={el:6.1f} s  speedup={t / el:5.1f}x  vol={volume[-1] / 1e6:.3f} Mm3 "
                f"max_depth_now={depth.max():.2f} m",
                flush=True,
            )
    wall = time.time() - t0
    hmax_solver = solver.Auxiliary.to_numpy()[:, :, 0]
    np.savez_compressed(
        wd / f"results{args.tag}.npz",
        t=np.array(ts),
        gauge_eta=np.array(gauge_eta),
        volume=np.array(volume),
        hmax=hmax.astype(np.float32),
        hmax_solver=hmax_solver,
        ...
        gauge_names=g["gauge_names"],
        gauge_i=gi,
        gauge_j=gj,
    )
    with open(wd / f"gauges{args.tag}.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["t_s", *g["gauge_names"]])
        for t, row in zip(ts, gauge_eta):
            w.writerow([f"{t:.2f}", *[f"{v:.3f}" for v in row]])
    print(
        f"done: {nsteps} steps, simulated {ts[-1]:.1f} s in {wall:.1f} s wall (realtime factor {ts[-1] / wall:.1f}x); "
        f"volume t0 {volume[0] / 1e6:.3f} -> end {volume[-1] / 1e6:.3f} Mm3",
        flush=True,
    )
